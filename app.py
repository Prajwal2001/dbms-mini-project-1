...
def login():
    ...
    if request.method == 'POST':
        ...
        cursor.execute(
            f'''SELECT * FROM {userType} WHERE {'docMailId' if userType == 'doctor' else 'mailId'} = '{mailId}' AND passwd= '{passwd}' ''')
        user = cursor.fetchone()
        if user:
            session['loggedIn'] = 1 if userType == 'patient' else (
                2 if userType == 'doctor' else 3)
...
def register():
    ...
    if request.method == 'POST':
        ...
        if cursor.fetchone():
            ...
            cursor.execute(
                f'''INSERT INTO patient VALUES ('{mailId}', '{passwd}', '{PName}', '{dob}', '{bloodGroup}', '{sex}')''')
            return redirect(url_for('login'))
...
def update():
    ...
    if 'loggedIn' in session:
        if request.method == 'POST':
            ...
            sex = request.form['sex']
            cursor.execute(
                f'''SELECT * FROM patient WHERE mailId = '{mailId}' ''')
            patient = cursor.fetchone()
            if patient:
                msg = 'Mail-Id already in use!'
            else:
                cursor.execute(
                    f'''UPDATE patient SET  mailId = '{mailId}', passwd = '{passwd}', PName = '{PName}', dob = '{dob}', bloodGroup = '{bloodGroup}', sex = '{sex}'  WHERE mailId = '{session['mailId']}' ''')
...
def adminLogin():
    ...
    if request.method == 'POST' and 'mailId' in request.form and 'passwd' in request.form:
        ...
        passwd = request.form['passwd']
        cursor.execute(
            f'''SELECT * FROM admin WHERE mailId = '{mailId}' and passwd= '{passwd}' ''')
...
def adminRegister():
    ...
    if request.method == 'POST' and 'mailId' in request.form and 'passwd' in request.form and 'adminName' in request.form:
        ...
        if receptionist:
            ...
            cursor.execute(
                'INSERT INTO receptionist VALUES (% s, % s, % s)', (mailId, passwd, adminName))
...
def adminDisplay():
    if 'loggedIn' in session:
        cursor.execute(
            f'''SELECT * FROM admin WHERE mailId = '{session['mailId']}' ''')
        admin = cursor.fetchone()
...
def adminUpdate():
    ...
    if 'loggedIn' in session:
        if request.method == 'POST':
            ...
            adminName = request.form['adminName']
            cursor.execute(
                f'''SELECT * FROM admin WHERE mailId = '{mailId}' ''')
            admin = cursor.fetchone()
            if not admin:
                msg = 'Account already exists !'
            else:
                cursor.execute(
                    f'''UPDATE admin SET  mailId = '{mailId}', passwd = '{passwd}', adminName = '{adminName}' WHERE mailId = '{session['mailId']}' ''')
                msg = 'You have successfully updated !'
        else:
            cursor.execute(
                f''' SELECT * FROM admin WHERE mailId = '{session['mailId']}' ''')
...
def doctorLogin():
    ...
    if request.method == 'POST' and 'docMailId' in request.form and 'passwd' in request.form:
        ...
        passwd = request.form['passwd']
        cursor.execute(
            f'''SELECT * FROM doctor WHERE docMailId = '{docMailId}' and passwd= '{passwd}' ''')
...
def doctorDisplay():
    if 'loggedIn' in session:
        cursor.execute(
            f'''SELECT * FROM doctor WHERE docMailId = '{session['mailId']}' ''')
        doctor = cursor.fetchone()
...
def myRecords():
    if 'loggedIn' in session:
        cursor.execute(
            f'''SELECT * FROM record WHERE mailId = '{session['mailId']}' ''')
...
def patientRecord():
    if 'loggedIn' in session:
        cursor.execute(
            f'''SELECT * FROM record WHERE mailId = '{session['patMailId']}' ''')
        record = cursor.fetchall()
        return render_template("patientRecord.html", loggedIn=session['loggedIn'], record=record)
    return redirect(url_for('home'))
...
def docAppointments():
    if 'loggedIn' in session:
        docMailId = True
        cursor.execute(
            f'''SELECT * FROM appointment WHERE docMailId = '{docMailId}' ''')
...
def doctorUpdate():
    ...
    if request.method == 'POST':
        ...
        cursor.execute(
            f'''SELECT * FROM doctor WHERE docMailId = '{docMailId}' ''')
        doctor = cursor.fetchone()
        if not doctor:
            msg = 'Account already exists !'
        else:
            cursor.execute(
                f'''UPDATE doctor SET  docMailId = '{docMailId}', passwd = '{passwd}', docName = '{docName}',availaibleDate= '{availableDate}' WHERE docMailId = '{session['mailId']}' ''')
            msg = 'You have successfully updated !'
    else:
        cursor.execute(
            f'''SELECT * FROM doctor WHERE docMailId = '{session['mailId']}' ''')
...
def patientUpdate(patMailId):
    ...
    if request.method == 'POST':
        ...
        if patient and patient[0] != patMailId:
            ...
            cursor.execute(
                f'''UPDATE patient SET  mailId = '{mailId}', passwd = '{passwd}', PName = '{PName}', dob = '{dob}', bloodGroup = '{bloodGroup}', sex = '{sex}'  WHERE mailId = '{patMailId}' ''')
        return redirect(url_for('patients'))
    else:
...
def doctorAdd():
    ...
    if request.method == 'POST':
        ...
        if doctor:
            ...
            cursor.execute(
                f'''INSERT INTO doctor(docMailId, passwd, docName, sex) VALUES ('{docMailId}', '{passwd}', '{docName}', '{sex}') ''')
            return redirect(url_for('home'))
...
def admindoctorUpdate(mailId):
    ...
    if request.method == 'POST':
        ...
        sex = request.form['sex']
        cursor.execute(
            f'''SELECT * FROM doctor WHERE docMailId = '{mailId}' ''')
        doctor = cursor.fetchone()
        if doctor and doctor[0] != mailId:
            msg = 'Mail-Id already in use!'
        else:
            cursor.execute(
                f'''UPDATE doctor SET  docMailId = '{docMailId}', passwd = '{passwd}', docName = '{docName}', sex = '{sex}' WHERE docMailId = '{mailId}' ''')
...
def nurseAdd():
    ...
    if 'loggedIn' in session and session['loggedIn'] == 3:
        if request.method == 'POST':
            ...
            cursor.execute(
                f'''SELECT * FROM nurse WHERE nurseId = '{nurseId}' ''')
            nurse = cursor.fetchone()
            if nurse:
                msg = "Nurse account already exists"
            else:
                cursor.execute('INSERT INTO nurse VALUES(%s,%s,%s)',
                               (nurseId, nurseName, phoneNumber))
...
def nurseUpdate(mailId):
    ...
    if request.method == 'POST':
        ...
        if nurse and nurse[0] != mailId:
            ...
            cursor.execute(
                f'''UPDATE nurse SET nurseId = '{nurseId}', phoneNumber = '{phoneNumber}', nurseName = '{nurseName}' WHERE nurseId = '{mailId}' ''')
        return redirect(url_for('nurses'))
    else:
        cursor.execute(f"SELECT * FROM nurse WHERE nurseId = '{mailId}'")
        nurse = cursor.fetchone()
...
def nurseAllocAdd():
    ...
    if 'loggedIn' in session:
        if request.method == 'POST':
            ...
            cursor.execute(
                f'''SELECT * FROM nursealloc WHERE mailId = '{mailId}' OR AND dateIn BETWEEN '{dateIn}' AND '{dateOut}' ''')
            ...
            if allocation:
                ...
                try:
                    cursor.execute(
                        f'''INSERT INTO nursealloc VALUES('{nurseId}','{mailId}','{dateIn}','{dateOut}') ''')
...
def appointmentAdd():
    ...
    if 'loggedIn' in session and session['loggedIn'] == 3:
        if request.method == 'POST':
            ...
            docMailId = request.form['docMailId']
            cursor.execute(
                f'''SELECT * FROM appointment WHERE mailId = '{mailId}' AND appointmentDate = '{appointmentDate}' AND docMailId = '{docMailId}' ''')
            appointment = cursor.fetchone()
            if appointment:
                msg = "Appointment already exists"
            else:
                cursor.execute('INSERT INTO appointment VALUES(%s,%s,%s)',
                               (mailId, appointmentDate, docMailId))
...
def appointmentUpdate(args):
    ...
    if request.method == 'POST':
        ...
        if appointment:
            ...
            cursor.execute(
                f'''UPDATE appointment SET mailId = '{mailId}', appointmentDate = '{appointmentDate}', docMailId = '{docMailId}' WHERE mailId = '{appoint[0]}' AND appointmentDate = '{appoint[1]}' AND docMailId = '{appoint[2]}' ''')
        return redirect(url_for('appointments'))
    else:
        cursor.execute(
            f"SELECT * FROM appointment WHERE mailId = '{appoint[0]}' AND appointmentDate = '{appoint[1]}' AND docMailId = '{appoint[2]}' ")
        appointment = cursor.fetchone()
...
def appointmentDelete(args):
    ...
    args = args.split()
    cursor.execute(
        f'''DELETE FROM appointment WHERE mailId = '{args[0]}' AND appointmentDate = '{args[1]}' AND docMailId = '{args[2]}' ''')
    return redirect(url_for('appointments'))
...
def recordAdd():
    if 'loggedIn' in session and session['loggedIn'] == 3:
        if request.method == 'POST':
            ...
            cursor.execute(
                f'''INSERT INTO record(mailId, Analysis) VALUES('{mailId}','{Analysis}')''')
...
def recordUpdate(recordId):
    ...
    if request.method == 'POST':
        ...
        Analysis = request.form['Analysis']
        cursor.execute(
            f'''SELECT * FROM record WHERE recordId = '{recordId}' ''')
        record = cursor.fetchone()
        if record and record[1] != recordId:
            msg = 'Record-Id already in use!'
        else:
            cursor.execute(
                f'''UPDATE record SET mailId = '{mailId}', Analysis = '{Analysis}' WHERE recordId = '{recordId}' ''')
        return redirect(url_for('records'))
    else:
        cursor.execute(f"SELECT * FROM record WHERE recordId = '{recordId}'")
        record = cursor.fetchone()
        cursor.execute(
            f'''SELECT mailId, Pname FROM patient WHERE mailId <> (SELECT mailId FROM record WHERE recordId='{recordId}') ''')
        patients = cursor.fetchall()
...
def testAdd():
    if 'loggedIn' in session and session['loggedIn'] == 3:
        if request.method == 'POST':
            ...
            cursor.execute(
                f'''INSERT INTO test(testName, testCategory) VALUES('{testName}', '{testCategory}')''')
...
def testUpdate(testId):
    ...
    if request.method == 'POST':
        ...
        if test and test[1] != testId:
            ...
            cursor.execute(
                f'''UPDATE test SET testName = '{testName}', testCategory = '{testCategory}' WHERE testId = {testId} ''')
        return redirect(url_for('tests'))
    else:
...
def diagnosisAdd():
    ...
    if 'loggedIn' in session and session['loggedIn'] == 3:
        if request.method == 'POST':
            ...
            analysis = request.form['analysis']
            cursor.execute(
                f'''SELECT * FROM diagnosis WHERE mailId = '{mailId}' AND testId = '{testId}' AND testDate = '{testDate}' ''')
            appointment = cursor.fetchone()
            if appointment:
                msg = "Diagnosis already exists"
            else:
                cursor.execute('INSERT INTO diagnosis VALUES(%s, %s, %s, %s)',
                               (mailId, testId, testDate, analysis))
...
def diagnosisUpdate(args):
    ...
    if request.method == 'POST':
        ...
        if diagnosis:
            ...
            cursor.execute(
                f'''UPDATE diagnosis SET mailId = '{mailId}', testId = '{testId}', testDate = '{testDate}', analysis = '{analysis}' WHERE mailId = '{test[0]}' AND testId = '{test[1]}' AND testDate = '{test[2]}' ''')
        return redirect(url_for('diagnosis'))
    else:
        cursor.execute(
            f"SELECT * FROM diagnosis WHERE mailId = '{test[0]}' AND testId = '{test[1]}' AND testDate = '{test[2]}' ")
        diagnosis = cursor.fetchone()
...
def diagnosisDelete(args):
    ...
    args = args.split()
    cursor.execute(
        f'''DELETE FROM diagnosis WHERE mailId = '{args[0]}' AND testId = '{args[1]}' AND testDate = '{args[2]}' ''')
    return redirect(url_for('diagnosis'))
...
def medicineAdd():
    if 'loggedIn' in session and session['loggedIn'] == 3:
        if request.method == 'POST':
            ...
            cursor.execute(
                f'''INSERT INTO medicine(medicineName) VALUES('{medicineName}')''')
...
def medicineDelete(medicineId):
    if 'loggedIn' not in session or session['loggedIn'] != 3:
        return redirect(url_for('home'))
    cursor.execute(
        f'''DELETE FROM medicine WHERE medicineId = '{medicineId}' ''')
    return redirect(url_for('medicines'))
...
def dosageAdd():
    ...
    if 'loggedIn' in session and session['loggedIn'] == 3:
        if request.method == 'POST':
            ...
            cursor.execute(
                f'''SELECT * FROM dosage WHERE mailId = '{mailId}' AND medicineId = '{medicineId}' AND quantity = '{quantity}' AND doseDate = '{doseDate}' ''')
            dosage = cursor.fetchone()
            if dosage:
                msg = "Entry already exists"
            else:
                cursor.execute('INSERT INTO dosage VALUES(%s,%s,%s,%s)',
                               (mailId, medicineId, quantity, doseDate))
                return redirect(url_for('dosages'))
        cursor.execute(
            f'''SELECT medicineId, medicineName FROM medicine ORDER BY medicineName ''')
        medicines = cursor.fetchall()
        cursor.execute(f'''SELECT mailId, Pname FROM patient ''')
        patients = cursor.fetchall()
...
def dosageUpdate(args):
    ...
    if request.method == 'POST':
        ...
        doseDate = request.form['doseDate']
        cursor.execute(
            f'''SELECT * FROM dosage WHERE mailId = '{mailId}' AND medicineId = '{medicineId}' AND doseDate = '{doseDate}' ''')
        dosage = cursor.fetchone()
        if dosage:
            msg = 'Appointment already exists!'
        else:
            cursor.execute(
                f'''UPDATE appointment SET mailId = '{mailId}', medicineId = '{medicineId}', quantity = '{quantity}', doseDate = '{doseDate}' WHERE mailId = '{dose[0]}' AND  medicineId = '{dose[1]}' AND doseDate = '{dose[2]}' ''')
        return redirect(url_for('dosages'))
    else:
        cursor.execute(
            f"SELECT * FROM appointment WHERE mailId = '{dose[0]}' AND  medicineId = '{dose[1]}' AND doseDate = '{dose[2]}' ")
        dosage = cursor.fetchone()
...
def dosageDelete(args):
    ...
    args = args.split()
    cursor.execute(
        f'''DELETE FROM dosage WHERE mailId = '{args[0]}' AND medicineId = '{args[1]}' AND doseDate = '{args[2]}' ''')
    return redirect(url_for('dosages'))
